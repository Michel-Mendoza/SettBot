...
module.exports = {
    ...
    emote: (client, name) => {
        ...
        let emote = emotes.find(e => e.name === name.split("'").join(''))
        if (!emote) {
            emote = emotes.find(e => e.name == name.split("'").join(''))
        }
    ...
    multisearch_txt: async (region, name, client) => {
        ...
        let champtext = `Campeón más jugado recientemente: ${client.functions.emote(client, (await client.functions.championIdentifiers(parseInt(main_champ[0], 10))).id)} **${champ} (${main_champ[1]} partidas)**`
